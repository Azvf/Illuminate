...
用这个 skill 诊断代码精简机会，判断何时消除冗余传递、缩短调用链。
...
| **B** | 抽成 `{Name}Provider` + DI 注入 | 数据生命周期长、传递链深（见下方门禁） |

### Provider 门禁

路径 B（提取 Provider）与 Constitution 默认拒绝 Provider/Adapter 等抽象存在张力。Provider 仅在以下条件**全部成立**时允许：
...
否则优先删除字段、缩短调用链或合并职责，不自动创建 Provider。
...
优先收拢到单一来源。仅当 Provider 门禁全部满足时才提取 Provider：
...
```kotlin
// √ Handler/Service 直接 inject Provider（需通过门禁）
...
```kotlin
// ❌ 函数上方需要长段注释才能解释它在做什么 → 说明函数职责爆炸
// Handle different states depending on player status:
// alive → tick, dead → schedule respawn, respawning → advance timer...
fun handleState(state: State) { ... }
```

```kotlin
// √ 拆分为职责单一的函数，命名即文档
fun handleAlivePlayer() { ... }
fun handleDeadPlayer() { ... }
fun handleRespawningPlayer() { ... }
```
...
```
指标 1-2 命中 → 数据属于全局上下文？
  ├─ Provider 门禁全部满足 → 路径 B：提取 Provider，收拢 fallback/转换逻辑
  └─ 门禁不满足 → 检查能否用参数或局部重构替代，否则 stop

指标 3-4 命中 → 链路设计问题
  └─ Provider 门禁满足 → 路径 B：Provider 注入到最终消费者，删除中间层透传
     门禁不满足 → 删除中间层透传，直接传参

指标 5 命中 → 门禁满足 → 合并为聚合 Provider
             门禁不满足 → 合并为单一配置对象直接传参
...
- **❌ 不满足门禁就创建 Provider** — 门禁五项缺一不可，否则用更直接的方案
