...
6. **Profile 类型对齐**：不同"慢"问题对应不同 profile 类型，不可一律只看总耗时
...
|------|---------|
| **cpu** | CPU 密集计算瓶颈 |
| **wall-clock** | 总耗时分析、阶段切分 |
| **allocation** | GC 压力、内存分配热点 |
| **io** | 文件/磁盘/数据库 I/O 瓶颈 |
| **frame-time** | UI 卡顿、掉帧、渲染管线 |
| **network** | 网络请求链路耗时 |

进入排查时必须先确认当前问题对应哪种 profile 类型，再选择对应工具。

## 工作流

1. **固定场景**：明确冷/热、首次/重复、具体操作链路
2. **确认 profile 类型**：根据现象判断应使用哪种 profile 类型
3. **锁定证据**：找到本次复现对应的日志、profile 文件
4. **跑脚本出报告**：优先用已有的 profile/分析脚本生成可读报告
5. **写三段结论**：当前现象、最重阶段、证据缺口
6. **缩小热点**：基于报告和日志把热点范围缩到具体 phase / batch / 时间窗
7. **定 root cause**：只有热点已锁定、日志已支撑时，才写根因判断
8. **决定后续**：修代码、补日志、补文档，而不是反过来先写方案
